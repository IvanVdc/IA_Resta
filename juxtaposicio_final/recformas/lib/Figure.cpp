...
ostream& operator<<(ostream &os, Figure &f)
{
  if (f.nvertices>0)
  {
    ...
    for (unsigned int i=0; i<f.infoVertices.size(); i++){
      //os<<f.infoVertices[i];
	int x = (int) f.infoVertices[i].pt.x;
	int y = (int) f.infoVertices[i].pt.y;
	fs << x <<endl;
	fs << y <<endl;

	}
	fs << f.centroid.x <<endl;
	fs << f.centroid.y <<endl;
	fs << f.RGB[0]<<endl;
	fs << f.RGB[1]<<endl;
	fs << f.RGB[2]<<endl;
	fs << f.color<<endl;
...
float Figure::menor_vertex_x(void){

    Point2D menor_x(99999999999.0, 0.0);
    for(int i; i<vertices.size();i++){
     if (vertices[i].x<menor_x.x){
         menor_x=vertices[i];
         }

        return float(menor_x.x);
    }
}
