...
void EdgeDetector::CannyDiscrete(vector<Point2D> * borderPts)
{
	...
	for (int y = support + 1; y < ny - support - 1; y++)
	{
		for (int x = support + 1; x < nx - support - 1; x++)
		{
			...
			if (gradient_noscale(x, y) > max_gradient)
				max_gradient = gradient_noscale(x, y);

			//***** gradient orientation *******//
			angle = (gradient_noscale(x, y) == 0.0) ? 0.0 : atan2(derivative[2](x, y),
					derivative[0](x, y));
			if (angle < 0.0)
				angle += M_PI;
			...
			if (derivative[0](x, y) > derivative[1](x, y) && derivative[0](x, y) > derivative[2](x, y)
					&& derivative[0](x, y) > derivative[3](x, y))
				dirmax(x, y) = 0; // E
			else if (derivative[1](x, y) > derivative[2](x, y) && derivative[1](x, y) > derivative[3](x,
					y))
				dirmax(x, y) = 1; // NE
	...
	for (int y = support + 2; y < ny - support - 2; y++)
	{
		for (int x = support + 2; x < nx - support - 2; x++)
		{
			...
			else
			{
				...
				outNMS(x, y) = ((derivative[dir](x, y) > derivative[dir1](x + dir_vector[dir][0], y
						+ dir_vector[dir][1]) && derivative[dir](x, y) >= derivative[dir2](x
						- dir_vector[dir][0], y - dir_vector[dir][1]))) ? 255.0f : 0.0f;
			}
		...
		for (int x = 0; x < w; x++)
		{
			...
			if (x == 0 || y == 0 || x == w - 1 || y == h - 1)
			{
				...
				else
				{
					...
					felzNeighbours.push_back(vector<int> ());
				}
			}
			else if (curCol != imRef(img,x-1,y) || curCol != imRef(img,x+1,y) || curCol
					!= imRef(img,x,y-1) || curCol != imRef(img,x,y+1)) // || curCol != imRef(img,x-1,y-1) || curCol != imRef(img,x+1,y+1) || curCol != imRef(img,x+1,y-1) || curCol != imRef(img,x-1,y+1))
			{
				count = 0;
				if (curCol == imRef(img,x-1,y-1))
					count++;
				if (curCol == imRef(img,x-1,y))
					count++;
				if (curCol == imRef(img,x-1,y+1))
					count++;
				if (curCol == imRef(img,x,y-1))
					count++;
				if (curCol == imRef(img,x,y+1))
					count++;
				if (curCol == imRef(img,x+1,y-1))
				...
				if (count >= 3)
				{
					...
					else
					{
						index = m.size();
						m[curCol] = index;
						v->push_back(vector<Point2D> (1, Point2D(x, y))); //Se crea un nuevo elemento en el vector, que corresponde a una figura
						felzNeighbours.push_back(vector<int> ()); //Se crea una lista de vecinos, y se relaciona con la figura por el indice del vector
					}

					// Buscamos vecino en x-1
					if (curCol != imRef(img, x-1, y))
					{
						int ind2;
						bool found;
						pos = m.find(imRef(img, x-1, y));
						if (pos != m.end())
						{
							ind2 = pos->second;
							if (index != ind2) //En index se encuentra el indice del curCol
							{
								found = false;
								// El vecino se trata de añadir en ambos indices
								for (int i = 0; i < felzNeighbours[index].size(); i++)
									if (felzNeighbours[index][i] == ind2)
										found = true;
								if (!found)
									felzNeighbours[index].push_back(ind2);

								found = false;
								for (int i = 0; i < felzNeighbours[ind2].size(); i++)
									if (felzNeighbours[ind2][i] == index)
										found = true;
								if (!found)
									felzNeighbours[ind2].push_back(index);
							}
						}
					}
					// Buscamos vecino en y-1
					if (curCol != imRef(img, x, y-1))
					{
						int ind2;
						bool found;
						pos = m.find(imRef(img, x, y-1));
						if (pos != m.end())
						{
							ind2 = pos->second;
							if (index != ind2)
